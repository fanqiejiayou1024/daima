        ...
        try:
            with open("config.json", encoding="utf-8") as f:
                ...
                for config in configs:
                    required = ["tenant_name", "account", "password"]
                    if not all(k in config for k in required):
                        raise ValueError(f"配置缺少必要字段: {required}")
                    # Validate study_time unit (seconds)
                    if "study_time" in config and config["study_time"] < 10:
                        raise ValueError("学习时长不能小于10秒")
                return configs
...
if __name__ == "__main__":
    ...
    for config in configs:
        ...
        study_time = config.get("study_time", 15)  # Unit: seconds
